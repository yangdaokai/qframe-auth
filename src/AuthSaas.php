 ...
 *      $auth=new AuthSaas();  $auth->check('规则名称','用户id')
 * 2，可以同时对多条规则进行认证，并设置多条规则的关系（or或者and）
 *      $auth=new AuthSaas();  $auth->check('规则1,规则2','用户id','and')
...
//            $REQUEST = unserialize(strtolower(serialize($this->request->param())));
    ...
    public function getAuthList($uid, $type = 1)
    {
        ...
        //读取用户组所有权限规则
        $rules = Db::name($this->config['auth_rule'])->where($map)->select();
        //循环规则，判断结果。
        $authList = []; //
